...
def get_movie_recommendation(movie_name):
	...
	if len(movie_list):        
		...
		return pd.DataFrame({'Title':[],'Distance':[]})
...
def new_movie():
	if request.method == 'GET':
		if bool(session):
			...
			return redirect(url_for('login'))
	elif request.method == 'POST' and 'movie' in request.form and 'rating' in request.form:
		...
		cursor11.execute('SELECT movieID,lower(title),genres FROM movies WHERE title = % s', (name, ))
		account = cursor11.fetchone()
		if account:
			msg = 'Movie already exists !'
		elif not re.match(r'[0-6]+', rating)or rating>'5':
			msg = 'Please enter valid rating !'
		elif not re.match(r'[0-9]+', year) or year>'2021':
			msg = 'Please enter valid year !'

		else:
			cursor12 = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
			cursor12.execute('SELECT max(movieID) FROM movies')
			hival=cursor12.fetchone()
			hiva=hival['max(movieID)']
			print(hiva)
			ma = int(hiva)
			ma=ma+1
			username = session['username']
			cursor12.execute('INSERT INTO movies VALUES (%s, % s, % s)', (ma, name, rating, ))
			mysql.connection.commit()
			cursor12 = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
			cursor12.execute('INSERT INTO user_movie VALUES (NULL, % s, % s, % s)', (username, name, rating, ))
			mysql.connection.commit()
			msg = 'New Movie Added !'
	elif not name or not rating:
		msg = 'Please fill out the form !'
	return render_template('new_movie.html', msg = msg)
